...
router.get("/", async (req, res, next) => {
  try {
    ...
    const groupId =
      groupIdQuery !== undefined ? Number(groupIdQuery) : undefined;

    if (groupId !== undefined && Number.isNaN(groupId)) {
      res
        .status(StatusCodes.BAD_REQUEST)
        .json("Invalid groupId filter");
      return;
    }

    const drones = await getAllDrones(groupId);
    res.status(StatusCodes.OK).json(drones);
  } catch (err) {
    next(err);
  }
});
  ...
  try {
    ...
    if (!drone) {
      res
        ...
        .json("Drone not found");
      return;
    }
    res.status(StatusCodes.OK).json(drone);
  } catch (err) {
    next(err);
  }
});
...
router.delete("/:id", async (req, res, next) => {
  try {
    ...
    res.status(StatusCodes.OK).json("Drone deleted successfully");
  } catch (err) {
    next(err);
  }
});
