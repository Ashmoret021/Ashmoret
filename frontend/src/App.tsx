...
export const App = () => {
  ...
  const [droneTypeOptions, setDroneTypeOptions] = useState<Array<{ id: number; name: string }>>([]);
  ...
  const handleSaveScenario = useCallback(async () => {
    ...
    const typeIdByName = new Map(
      droneTypeOptions.map((type) => [normalizeTypeName(type.name), type.id] as const)
    );
    ...
    const payload = {
      ...
      drones: normalizedDrones.filter((drone): drone is NonNullable<typeof drone> => drone !== null),
    };
  ...
  }, [attackName, attackDescription, droneTypeOptions, placedDrones, resetAttackDraft, showToast, waves]);
